...
export class TableParser {
    static parseForeignKey(table, tables, scheme) {
        ...
        for (let i = 0; i < table.columns.length; i++) {
            ...
            if (column.name === foreignKey) {
                const newColumn = new ColumnSchema(column.name, column.type, column.modifiers, column.typeSize, column.primaryKey, refColumn, column.default, column.primaryKeyTypeSize);
    ...
    static parsePrimaryKey(table, tables, schema) {
        ...
        for (const columnName of exec[1].split(/[,\s]+/)) {
            ...
            const column = new ColumnSchema(foundColumn.name, foundColumn.type, foundColumn.modifiers, foundColumn.typeSize, true, foundColumn.foreignKey, foundColumn.default, keySize);
    ...
    static parseUnique(table, tables, data) {
        ...
        for (const columnName of exec[1].split(/[,\s]+/)) {
            ...
            const column = new ColumnSchema(foundColumn.name, foundColumn.type, foundColumn.modifiers, foundColumn.typeSize, false, foundColumn.foreignKey, foundColumn.default, keySize);
