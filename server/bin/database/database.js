 ...
 */
export async function inContext(callback, transaction = true) {
    if (!running) {
...
function start() {
    if (!running) {
        ...
        try {
            ...
            startPromise = inContext((context) => StateProcessor.checkTableSchema(context), true).catch((error) => {
                logger.error(error);
                console.log(error);
                errorAtStart = true;
...
export const Storage = {
    ...
    moveMedium(oldListId, newListId, mediumId, uuid) {
        return inContext((context) => context.setUuid(uuid).moveMedium(newListId, oldListId, mediumId));
    },
    getSimpleMedium(id) {
