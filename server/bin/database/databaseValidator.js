...
import { ColumnType, InvalidationType, Modifier } from "./databaseTypes";
        ...
        for (const table of this.tables) {
            for (const invalidation of table.invalidations) {
                ...
                if (invalidation.type & InvalidationType.INSERT) {
                    const trigger = createTrigger(watchTable, table, invalidationTable, mainPrimaryKey, columnConverter, InvalidationType.INSERT);
                    this.trigger.push(trigger);
                }
                if (invalidation.type & InvalidationType.UPDATE) {
                    const trigger = createTrigger(watchTable, table, invalidationTable, mainPrimaryKey, columnConverter, InvalidationType.UPDATE);
                    this.trigger.push(trigger);
                }
                if (invalidation.type & InvalidationType.DELETE) {
                    const trigger = createTrigger(watchTable, table, invalidationTable, mainPrimaryKey, columnConverter, InvalidationType.DELETE);
                    this.trigger.push(trigger);
                }
            }
        }
...
export const StateProcessor = StateProcessorImpl;
