...
import { allTypes, BcryptHash, combiIndex, equalsIgnore, Errors, getElseSet, Hashes, MediaType, multiSingle, promiseMultiSingle, sanitizeString, separateIndex } from "../tools";
...
export class QueryContext {
    ...
    async getList(listId, media, uuid) {
        ...
        // @ts-ignore
        const lists = await promiseMultiSingle(listId, async (id) => {
    ...
    async createFromMediaInWait(medium, same, listId) {
        ...
        if (same && Array.isArray(same)) {
            ...
            const synonyms = same.map((value) => sanitizeString(value.title))
    ...
    async deleteMediaInWait(mediaInWait) {
        ...
        // @ts-ignore
        return promiseMultiSingle(mediaInWait, (value) => this._delete("medium_in_wait", {
    ...
    async getMediumPartsPerIndex(mediumId, index, uuid) {
        const parts = await this._queryInList("SELECT * FROM part " +
            `WHERE medium_id = ${mySql.escape(mediumId)} AND combiIndex `, index);
    ...
    async addPart(part) {
        ...
        try {
            ...
            const result = await this._query("SELECT id from part where medium_id=? and combiIndex=?", [part.mediumId, combiIndex(part)]);
            partId = result[0].id;
        }
    ...
    updateRelease(releases) {
        // @ts-ignore
        return promiseMultiSingle(releases, async (value) => {
            if (value.episodeId) {
    ...
    addEpisode(episodes) {
        ...
        // @ts-ignore
        return promiseMultiSingle(episodes, async (episode) => {
            let insertId;
            try {
                ...
                const result = await this._query("SELECT id from episode where part_id=? and combiIndex=?", [episode.partId, combiIndex(episode)]);
    ...
    async getEpisode(id, uuid) {
        const episodes = await this._queryInList("SELECT * FROM episode LEFT JOIN user_episode ON episode.id=user_episode.episode_id " +
            `WHERE (user_uuid IS NULL OR user_uuid=${mySql.escape(uuid)}) AND episode.id`, id);
        if (!episodes || !episodes.length) {
            return [];
        }
        const idMap = new Map();
    ...
    async getPartEpisodePerIndex(partId, index) {
        const episodes = await this._queryInList("SELECT * FROM episode " +
            `where part_id =${mySql.escape(partId)} AND combiIndex`, index);
        if (!episodes || !episodes.length) {
            return [];
        }
    ...
    async getMediumEpisodePerIndex(mediumId, index) {
        const episodes = await this._queryInList("SELECT episode.* FROM episode INNER JOIN part ON part.id=episode.part_id " +
            `WHERE medium_id =${mySql.escape(mediumId)} AND episode.combiIndex`, index);
        if (!episodes || !episodes.length) {
            return [];
        }
        const availableIndices = [];
    ...
    setProgress(uuid, progressResult) {
        // @ts-ignore
        return promiseMultiSingle(progressResult, async (value) => {
    ...
    removeSynonyms(synonyms) {
        // @ts-ignore
        return promiseMultiSingle(synonyms, (value) => {
    ...
    async addSynonyms(synonyms) {
        ...
        multiSingle(synonyms, (value) => {
            // @ts-ignore
            multiSingle(value.synonym, (item) => {
    ...
    async _delete(table, ...condition) {
        ...
        const values = [];
        multiSingle(condition, (value, _, last) => {
    ...
    async _update(table, column, idValue, cb) {
        ...
        values.push(idValue);
        const result = await this._query(`UPDATE ${mySql.escapeId(table)}
                SET ${updates.join(", ")}
                WHERE ${mySql.escapeId(column)} = ?;`, values);
        return result.affectedRows > 0;
    }
