...
import requestNative from "request";
...
function processRequest(uri, otherRequest, queueToUse = queues, limit) {
    ...
    const toUseRequest = otherRequest || request;
    return { toUseRequest, queue };
}
export const queueRequest = (uri, options, otherRequest) => {
    const { toUseRequest, queue } = processRequest(uri, otherRequest);
    return queue.push(() => toUseRequest.get(uri, options));
};
export const queueCheerioRequestBuffered = (uri, options, otherRequest) => {
    const { toUseRequest, queue } = processRequest(uri, otherRequest);
    if (!options) {
        options = { uri };
    }
...
function streamParse5(resolve, reject, uri, options) {
    ...
    const parser = new ParserStream({ treeAdapter: parse5.treeAdapters.htmlparser2 });
    parser.on("finish", () => {
...
function streamHtmlParser2(resolve, reject, uri, options) {
    ...
    const parser = new htmlparser2.WritableStream(new DomHandler((error, dom) => {
        // @ts-ignore
...
export const queueCheerioRequestStream = (uri, options) => {
...
const transformCheerio = (body) => cheerio.load(body);
...
export const queueRequestFullResponse = (uri, options, otherRequest) => {
    return queueFullResponseWithLimit(uri, options, otherRequest);
};
export const queueFastRequestFullResponse = (uri, options, otherRequest) => {
...
export const queueWork = (key, callback) => {
    return queueWithLimit(key, callback);
};
export const queueFastWork = (key, callback) => {
