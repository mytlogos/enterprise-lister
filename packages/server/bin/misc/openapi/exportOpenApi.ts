      ...
      if (!identifier) {
        ...
        if (!callSymbol) {
          ...
          const signature = this.checker.getSignatureFromDeclaration(
            callSymbol.valueDeclaration as FunctionDeclaration,
          );
...
class Parser {
  ...
  private searchMiddlewareStack(container: SearchContainer): void {
    ...
    if (!requestParamSymbol && !responseParamSymbol) {
      log("Faulty Function: Missing either Request and Response Parameter: " + container.middleware.getText());
      return;
    }
    ...
    if (ts.isArrowFunction(middlewareSignature)) {
      ...
      (ts.isFunctionDeclaration(middlewareSignature) || ts.isFunctionExpression(middlewareSignature)) &&
      middlewareSignature.body
    ) {
  ...
  private searchMiddlewareStatement(body: ts.Node, container: SearchContainer) {
      ...
      if (ts.isCallExpression(t)) {
        for (const argument of t.arguments) {
          if (ts.isIdentifierOrPrivateIdentifier(argument)) {
            return (
              this.isSymbol(argument, container.currentStackElement.requestSymbol) ||
    ...
    for (const callExpression of callsWithEitherReqOrRes) {
      if (ts.isIdentifierOrPrivateIdentifier(callExpression.expression)) {
        ...
        if (!ts.isFunctionLike(declaration)) {
          log("Value Declaration of Call Symbol is no SignatureDeclaration: " + callExpression.expression.getText());
          continue;
        }
    ...
    while (!end && target) {
      if (
        (ts.isCallExpression(target) &&
          this.getCalledName(target) === "catch" &&
          target.arguments.includes(previousTarget as ts.Expression)) ||
        ts.isCatchClause(target)
      ) {
        end = true;
        errorBlock = true;
  ...
  private getReturnRouter(
    outerFunction: ts.FunctionDeclaration,
    variableSymbol: Nullable<ts.Symbol>,
  ): Optional<ts.Symbol> {
    if (!outerFunction.body) {
      return;
    }
    const returnNodes = filterKind(outerFunction.body.statements, SyntaxKind.ReturnStatement).filter((value) => {
      const identifier = getFirst(value, SyntaxKind.Identifier);
      if (!identifier) {
        return false;
      }
      const identifierSymbol = this.checker.getSymbolAtLocation(identifier);
      return identifierSymbol && (this.routerVariables.has(identifierSymbol) || identifierSymbol === variableSymbol);
    });
    ...
    if (firstCallIdentifier) {
      ...
      if (symbol) {
        if (symbol.getName() === "Router") {
          const outerFunction = getFirstParent(node, ts.SyntaxKind.FunctionDeclaration) as ts.FunctionDeclaration;
  ...
  private processRouter(
    propertyName: ts.Identifier | ts.PrivateIdentifier,
    node: ts.Node,
    variableSymbol: ts.Symbol,
    variableIdentifier: ts.LeftHandSideExpression,
  ) {
   ...
   * @param node
   */
  private processUseMiddleware(
    parameterList: ts.SyntaxList,
    variableIdentifier: ts.LeftHandSideExpression,
    variableSymbol: ts.Symbol,
    node: ts.Node,
  ) {
  ...
  public inspectCall(node: ts.Node) {
    const propertyAccess = getFirst(node, ts.SyntaxKind.PropertyAccessExpression) as PropertyAccessExpression;
...
function setPathObject(
  ...
  genericParams: Record<string, null>,
): OperationObject {
...
function setRequestBody(
  pathRequestObject: RequestBodyObject,
  genericParameters: Record<string, null>,
  bodyType: Record<string, TypeResult>,
): void {
