...
export function parseForeignKey(table: TableSchema, tables: TableSchema[], scheme: string): void {
...
export function parsePrimaryKey(table: TableSchema, _tables: TableSchema[], schema: string): void {
  ...
  for (const columnName of exec[1].split(/[,\s]+/)) {
    ...
    const column = new ColumnSchema(
      foundColumn.name,
      foundColumn.type,
      foundColumn.modifiers,
      foundColumn.typeSize,
      true,
      foundColumn.foreignKey,
      foundColumn.default,
      keySize,
    );
...
export function parseUnique(table: TableSchema, _tables: TableSchema[], data: string): void {
...
export function parseDataColumn(table: TableSchema, _tables: TableSchema[], value: string): Nullable<ColumnSchema> {
...
function getExpressionValue(columnParts: string[], index: number): { index: number; value: string } {
  ...
  if (defaultValue.startsWith("(")) {
    ...
    for (; index < columnParts.length; index++) {
      ...
      if (currentValue.startsWith("(")) {
        defaultValue += getExpressionValue(columnParts, index).value;
