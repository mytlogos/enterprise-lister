<template>
  <div ref="root">
    <div class="d-flex flex-column dropdown-container">
      ...
      <button class="btn search btn-dark btn-sm" type="submit" @click="showSearch = !showSearch">
        <img alt="Search" src="../assets/search_icon.png" />
      </button>
      <div class="dropdown dropdown-btn" :class="{ hidden: !showSearch }">
        <input v-model="filter" placeholder="Type your Search..." type="text" />
      </div>
    </div>
    <list-comp :data="displayedData" :filter="filter" :focused="listFocused" :multi="true" @select="selectList" />
  </div>
</template>
