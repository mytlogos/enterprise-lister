...
export function resultFromQuery(query: Query): Promise<any[]> {
    return new Promise((resolve, reject) => {
        const rows: any[] = [];
        let rejected = false;
        query.on("result", (row: unknown) => rows.push(row))
            .on("error", (error) => {
                rejected = true;
                reject(error);
            })
...
export async function fillUserTable(): EmptyPromise {
    const dummy = user[0];
    await inContext(context => context.query(
        "INSERT IGNORE INTO user (name, uuid, password, alg) VALUES (?,?,?,?);",
        [dummy.name, dummy.uuid, dummy.password, dummy.alg]
    ));
...
export async function fillMediumTable(): EmptyPromise {
    ...
    await inContext(context => context.query(
        "INSERT IGNORE INTO medium (id, medium) VALUES (?,?);",
...
export async function fillPartTable(): EmptyPromise {
    ...
    await inContext(context => context.query(
        "INSERT IGNORE INTO part (id, medium_id, totalIndex) VALUES (?,?,?);",
        [dummy.id, dummy.mediumId, dummy.totalIndex]
    ));
...
export async function fillEpisodeTable(): EmptyPromise {
    ...
    await inContext(context => context.query(
        "INSERT IGNORE INTO episode (id, part_id, totalIndex) VALUES (?,?,?);",
        [dummy.id, dummy.partId, dummy.totalIndex]
    ));
...
export async function fillEpisodeReleaseTable(): EmptyPromise {
    ...
    await inContext(context => context.query(
        "INSERT IGNORE INTO episode_release (episode_id, url, title, releaseDate) VALUES (?,?,?,?);",
        [dummy.episodeId, dummy.url, dummy.title, dummy.releaseDate]
    ));
...
export async function fillUserEpisodeTable(): EmptyPromise {
    ...
    await inContext(context => context.query(
        "INSERT IGNORE INTO user_episode (user_uuid, episode_id, progress) VALUES (?,?,?);",
        [dummy.uuid, dummy.episodeId, dummy.progress]
    ));
